...
const UpdateProductPage = () => {
  ...
  const getProduct = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/products/show/${id}`,
  ...
  const onSubmit: SubmitHandler<FormValues> = async (values) => {
    try {
      ...
      productData.append("modby", session?.user?.name ?? "admin");
      productData.append("modtime", new Date().toISOString());

      if (values.image instanceof File) {
        productData.append("image", values.image);
      } else if (typeof values.image === "string") {
        productData.append("imagePath", values.image);
      }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <div className="space-y-2">
                      ...
                      <div
                        ...
                        className="cursor-pointer inline-block border rounded-md overflow-hidden hover:opacity-80 transition"
                      >
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img
                          src={`${process.env.NEXT_PUBLIC_API_URL}${field.value}`}
                          alt={"preview"}
                          width={100}
                          height={100}
                  ...
                  <FormDescription>
                    Klik gambar untuk mengganti file (format .jpg, .png, .jpeg).
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
